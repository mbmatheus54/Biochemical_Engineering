...
def get_pdb(*code: str):
    ...
    for x in code:
        url = []
        url.append(f'https://files.rcsb.org/download/{x.upper()}.pdb')
        for y in url:
            status = requests.get(y)
            if status.status_code == 200:
                print('[-] Acesso ao servidor: OK!')
                request = requests.get(y, stream=True)
                with open(f'pdb_archive-{x}', 'wb') as file:
                    copyfileobj(request.raw, file)
            else:
                print('[-] Acesso ao servidor: NEGADO!')


def get_fasta(*code: str):
    global x, url

    for x in code:
        url = []
        url.append(f'https://www.rcsb.org/fasta/entry/{x.upper()}')
        for y in url:
            status = requests.get(y)
            if status.status_code == 200:
                print('[-] Acesso ao servidor: OK!')
                request = requests.get(y, stream=True)
                with open(f'fasta_archive-{x}', 'wb') as file:
                    copyfileobj(request.raw, file)
            else:
                print('[-] Acesso ao servidor: NEGADO!')
